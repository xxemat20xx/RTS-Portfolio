    ...
    <section className="relative min-h-[90vh] flex items-center justify-center px-6 overflow-hidden">
      ...
      <div className="max-w-7xl w-full grid md:grid-cols-2 gap-16 items-center relative z-10">
        <Motion.div variants={container} initial="hidden" animate="visible" className="space-y-8">
          ...
          <Motion.p variants={fadeUp} className="text-gray-400 text-lg md:text-xl max-w-xl leading-relaxed">
          ...
          <Motion.div variants={fadeUp}>
            <h2 className="text-sm uppercase tracking-widest text-gray-500 mb-4">Tech Stack</h2>
            <Motion.div variants={container} initial="hidden" animate="visible" className="flex flex-wrap gap-5">
              {techStackImages.map((tech) => (
                <Motion.div
                  key={tech.id}
                  variants={fadeUp}
                  whileHover={{ y: -6 }}
                  className="group p-3 bg-gray-800/60 backdrop-blur rounded-xl hover:bg-violet-600/20 transition duration-300 shadow-md"
                >
                  <img
                    src={tech.icon}
                    alt={tech.name}
        ...
        <Motion.div {...floatAnimation} animate="animate" className="hidden lg:flex justify-center relative">
