...
const Hero = () => {
    ...
    return (
        <section className='hero-section mx-auto flex justify-center items-center'>
            <div className='grid grid-cols-1 md:grid-cols-2'>
                {/* Hero Content */}
                <div
                    className="hero-content order-2 md:order-1 flex flex-col items-start justify-center"
                >
                    <h1 className='text-4xl font-bold mb-4 md:text-6xl'>Hi I'm <span className='text-violet-500 font-extrabold'>{heroData.name}</span></h1>
                    <p className='text-gray-400 mb-4 max-w-3xl'>{heroData.heroParagraph}</p>
                    <div className='my-4'>
                        <button className='bg-violet-500 text-white px-4 py-2 rounded-full hover:bg-violet-600 transition-all duration-300 cursor-pointer'>More about me &rarr;</button>
                    </div>
                    {/* Tech Stack */}
                    ...
                    <div className="tech-stack flex flex-wrap gap-4">
                        {techStackImages.map((image) => (
                            ...
                            >
                                <img src={image.techStackImage} alt="" className='w-10 h-10' />
                            </div>
                        ))}
                    </div>
                </div>
